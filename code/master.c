...
int main(int argc, char *argv[])
{
	...
	if (argc != 11)
	{
		printf("Usage: ./master –w numWorkers -b bufferSize –s serverIP –p serverPort -i input_dir\n");
		return -1;
	}
  ...
  //   		perror("fnctl fifoW");
	...
	for (int i = 0; i < numWorkers; i++)
	{
		int size = (int) strlen(serverIP) + 1;
        // printf("sizeOfMessage dA  -- %d\n", size);
       	//write size of message 
       	if (write(fifosW[i], &size, sizeof(int)) == -1){ 
			perror("write size");
	    ...
	    if (write(fifosW[i], serverIP, size) == -1){ 
			perror("write serverIP");
			return -1;
		}

		if (write(fifosW[i], &serverPort, sizeof(int)) == -1){ 
			perror("write serverPort");
			return -1;
		}
